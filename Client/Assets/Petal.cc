...
void draw_static_petal_single(PetalID::T id, Renderer &ctx) {
    ...
    switch(id) {
        ...
        case PetalID::kSoil: {
            ...
            ctx.begin_path();
            ctx.move_to(-6.667, -5.429);
            ctx.line_to(1.905, -9.905);
            ctx.line_to(8.571, -5.429);
            ctx.line_to(9.524, 5.619);
            ctx.line_to(0, 10.762);
            ctx.line_to(-6.667, 8.191);
            ctx.line_to(-9.524, 1.238);
            ctx.close_path();
            ctx.fill();
            ctx.set_line_width(3);
            ctx.stroke();
